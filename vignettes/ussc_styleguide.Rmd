...
How we present data *matters*. Our primary goal at USSC is to present our information in an aesthetic,  scientifically rigourous manner. We aim to convey our message as clearly and as cleanly as we can. As Edward Tufte (one of the pioneers of data visualisation) wrote, "Confusion and clutter are failures of design, not attributes of information" [@tufte1990envisioning, p. 53]. It is important to remember that visualising data is not for our benefit as researchers, but rather for those who consume our work.

With that being said, there are a few key points to keep in mind if you're working with data.

1. Given that we are an think tank working on empirical questions (or, at least, one hopes...), our visualisation work must match the standard of 'good' data visualization and ideally go far beyond that level.
2. This means following the best practices in data visualisation and, more broadly, statistics.
3. There are several guides for effectively visualising data. Edward Tufte's books, for example, are extremely helpful. 
...
ggplot(iris, aes(Sepal.Width, Sepal.Length, colour = Species)) + 
  ...
  annotate("text", x = 2.3, y = 7.2, label = "Axis - title & text font size: 11,\nfont colour: #444444\ntick colour: #D3D3D3", colour = "purple") +
   geom_curve(aes(x = 2.2, y  = 7.7, xend = 1.9, yend = 8),
    ...
    arrow = arrow(length = unit(0.03, "npc")),
    colour = "purple") +
  annotate("text", x = 4.2, y = 7.4, label = "Grid lines - \ncolour: #EBEBEB,\nmajor grid line width = 0.5pt,\nminor gridline width = 0.25pt", colour = "purple") + 
   geom_curve(aes(x = 3.7, y  = 7.3, xend = 3.52, yend = 7.9),
    curvature = 0.2,
    arrow = arrow(length = unit(0.03, "npc")),
    colour = "purple")+ 
   geom_curve(aes(x = 3.65, y  = 7, xend = 3.4, yend = 7.45),
    curvature = -0.4,
    arrow = arrow(length = unit(0.03, "npc")),
    colour = "purple")+
...
* The main content width for the USSC website is **780 pixels (px)** wide, which translates to **8.125 inches (in)** or **20.636 centimetres (cm)**. When saving a publication-ready graph, set the width to **8 inches** (**20 cm** or **780px**). Most graphs fit in this div.
...
* Avoid plotting graphs with dual y-axes. Split them up! People are already confused by numbers and statistics. Our aim is to simplify our presentation so that everyone can get the key point. Plotting two graphs together on the same panel invites confusion. From first glance, it can be hard to tell which line belongs to what. [More on this from the team at datawrapper,](https://blog.datawrapper.de/dualaxis/) as well as a few pieces from [notable](http://www.perceptualedge.com/articles/visual_business_intelligence/dual-scaled_axes.pdf) [statisticians](https://stackoverflow.com/questions/3099219/plot-with-2-y-axes-one-y-axis-on-the-left-and-another-y-axis-on-the-right/3101876#3101876) [and](https://kieranhealy.org/blog/archives/2016/01/16/two-y-axes/) [social](http://www.storytellingwithdata.com/blog/2016/2/1/be-gone-dual-y-axis) [scientists](https://stephanieevergreen.com/two-alternatives-to-using-a-second-y-axis/) (NB: they are all individual links!). [Here's an academic study detailing why dual y-axis suck.](https://www.lri.fr/~isenberg/publications/papers/Isenberg_2011_ASO.pdf)


* Side note: I can't even create dual axis charts in `ggplot2` - Hadley was quite clear (see above) about why he hasn't given users the option do so (because, uhhh, they're terrible and pretty much fall into the statistics-as-lies category). 
...
* Please do *not* use a rainbow palette. 
...
* Direct labels are easier to understand and are quicker to read. This is really important from an accessibility standpoint. People who are colour blind might struggle to match a legend key to a specific point on the graph.
...
* First, choropleth maps are often viewed as problematic because geographic areas and population vary in size, so maps might mislead the viewer
...
* Tip: normalise the data before plotting
...
* A note on pie charts from Tufte:
