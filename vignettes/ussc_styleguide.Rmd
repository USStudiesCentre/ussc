...
* Remove axis
* Remove colour
* Simplify axis labels; remove them if they are redundant. If you can not simplify x-axis labels, flip the chart so the reader can read the x-axis label (now the y-axis label) easily.
...
* A quick note on vertical x-axis: they are cramped, difficult to read and should be avoided. 
  ...
  * Third, consider whether you need *all* of the labels. For example, if you're plotting election results, you do not need to plot every single year along the x-axis -- simply plot election years. 
...
* In the plot above, we plotted the in-party approval rating for 12 US presidents in their first year in office, from Eisenhower to Trump. It is impossible to see the data clearly - this is the perfect example of a *spaghetti chart*. Note: time series data tend to look like spaghetti charts when you have >5 lines. There are ways to fix this issue. Maybe we're only interested in President Trump. We can selectively highlight Trump's in-party approval rating.
...
ggplot(pid_data) + 
  ...
  theme_ussc() + 
  labs(title = "In-party approval rating for American presidents from 1956-2018",
       subtitle = "President Eisenhower to President Trump\nTrump's in-party approval rating is highlighted in red.", 
       x = "# of days into first term", 
       y = NULL) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 2))
```

* What if, instead of emphasising one President, we want to compare all Presidents from 1956 onwards? We can easily do this by splitting data into several small charts.
