...
With that being said, there are a few key points to keep in mind if you're working with data.First, given that we are an think tank working on empirical questions, our visualisation work must match the standard of 'good' data visualization and ideally go far beyond that low benchmark. This means following the best practices in data visualisation and, more broadly, statistics. There are several guides for effectively visualising data. Edward Tufte's books, for example, are extremely helpful. 
...
[People use statistics and graphs to distort data in misleading ways.](https://en.wikipedia.org/wiki/Misleading_graph) How do they do this? By deliberately creating misleading graphs. Because many are statistically innumerate, this is, quite frankly, a good tactic if you want to convince someone that you're right above all else. Data, statistics, graphs all add credence your argument and people are reluctant to critique empirical evidence because a) they don't know how to and b) they probably make the assumption that the creator is more statistically knowledgeable than they are. But a major contributing factor to misleading graphs is the innumeracy of the data viz creators themselves. The proliferation of tools like Excel, graphic design tools, and other data viz tools lead anyone to think they can create a graph (just like anyone can write the next great American novel!). However, a lot of these people lack basic statistical skills which lead to obvious mistakes. It means that there are *plenty* of examples of truly shocking data viz out there. 
...
* Sort your data in a logical (i.e. ascending, descending) order before plotting. 


* Really think about colour. We use colour to differentiate between groups or layers. If all bars measure the same variable, use the same colour. If you want to highlight one group -- colour that specific bar a bright colour (for example <span style="background-color:#ED1B35;color:white;font-weight:bold;";>red</span> or <span style="background-color:#009DE3;color:white;font-weight:bold;";>light blue</span>), leaving all others grey. 
