...
ggplot(mtcars, aes(x=wt, y=mpg, colour = as.factor(cyl))) + 
  ...
  annotate("text", x = 2.3, y = 12.2, label = "Axis - title & text font size: 11,\nfont colour: #444444\ntick colour: #D3D3D3", colour = "purple") +
   geom_curve(aes(x = 1.7, y  = 14.7, xend = 1.5, yend = 16),
    ...
    arrow = arrow(length = unit(0.03, "npc")),
    colour = "purple") +
  annotate("text", x = 3.5, y = 25.4, label = "Grid lines - \ncolour: #EBEBEB,\nmajor grid line = 0.5pt,\nminor gridline = 0.25pt", colour = "purple") + 
   # geom_curve(aes(x = 3.8, y  = 20.3, xend = 3.53, yend = 20.95),
   #  curvature = 0.2,
   #  arrow = arrow(length = unit(0.03, "npc")),
   #  colour = "purple")+
   # geom_curve(aes(x = 3.8, y  = 23, xend = 3.4, yend = 23.45),
   #  curvature = -0.4,
   #  arrow = arrow(length = unit(0.03, "npc")),
   #  colour = "purple")+
...
Another contributing factor to the proliferation of misleading graphs is the innumeracy of data viz creators themselves. The popularity of tools like Excel, graphic design programs, and other data viz packages lead anyone to think they can create a graph (just like anyone can write the next great American novel!). However, many lack basic statistical skills which lead to obvious mistakes. Edward Tufte and others in the visualisation space have written on this extensively. Indeed, Tufte wrote, "Lurking behind the inept graphic is a lack of judgement about quantitative evidence. ... Illustrators too often see their work as an exclusively artistic entreprise. ... Those who get ahead are those who beautify data, never mind statistical integrity." [-@tufte2014visual, p79]. Yet some of the worst graphs have come from scientists, engineers, computer scientists or economists. It means that there are *plenty* of examples of truly shocking data viz out there but *we should hold ourselves to a higher standard*. 

Nathan Yau (a prolific statistician and data visualiser) wrote about misleading viz [here](https://flowingdata.com/2017/02/09/how-to-spot-visualization-lies/). Michael Correll, a research scientist at Tableau, and Jeffrey Heer, a professor of data visualization and human computer interaction, recently wrote a piece differentiation between *malicious* misleading visualizations and just plain *incorrect* visualizations [here](https://idl.cs.washington.edu/files/2017-BlackHatVis-DECISIVe.pdf). 
